...
def load_custom_dataset(data_dir, split_name, emotion):
    data = []
    ...
    for speaker_dir in os.listdir(data_dir):
        ...
        # Check if it's a directory (i.e., a speaker)
        if os.path.isdir(speaker_path):
            split_path = os.path.join(speaker_path, split_name)

            # Loop through each file in the split directory
            for filename in os.listdir(split_path):
                if filename.endswith(".wav"):
                    if filename.split("_")[1] != emotion:
                        continue
                    wav_path = os.path.join(split_path, filename)
                    txt_path = os.path.splitext(wav_path)[0] + ".txt"

                    # Read the text file for transcription and emotion
...
def main():
    ...
    if training_args.do_train:
        raw_datasets["train"] = load_custom_dataset(
    ...
    if training_args.do_eval:
        raw_datasets["eval"] = load_custom_dataset(
            data_args.dataset_name,
    ...
    def prepare_dataset(batch):
        ...
        #     labels.append(audio_inputs.get("input_features")[0])
        #     mel_scaled_input_features.append(audio_inputs.get("mel_scaled_input_features")[0])
        ...
        #     input_str = texts[i].lower() if do_lower_case else texts[i]
        
        #     if is_uroman:
        #         input_str = uromanize(input_str, uroman_path=uroman_path)
        #     string_inputs = tokenizer(input_str, return_attention_mask=False)

        #     names.append(string_inputs.get("input_ids")[: max_tokens_length + 1])
        #     lengths.append(len(names[i]))
        #     waveforms.append(batch[audio_column_name][i]["array"])

        # print(f"New number of speakers are {new_num_speakers}")
        # if speaker_id_column_name is not None:
        #     if new_num_speakers > 1:
        #         print("HEEEEEEEEEEEEEEEEEY")
        #         # align speaker_id to [0, num_speaker_id-1].
        #         speakers = speaker_id_dict.get(batch[speaker_id_column_name], 0)
        
        # return {
        #     "labels": labels,
        #     "tokens_input_length": lengths,
        #     model_input_name: names,
        #     "waveform": waveforms,
        #     "mel_scaled_input_features": mel_scaled_input_features,
        #     "speaker_id": speaker_id_dict.get(batch[speaker_id_column_name], 0) if new_num_speakers > 1 else batch[speaker_id_column_name]
        # }
        # process target audio
